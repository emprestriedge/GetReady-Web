...
export const SpotifyDataService = {
  ...
  getLikedTracks: async (limit = 20, offset = 0): Promise<SpotifyTrack[]> => {
    ...
    // FIX: Filter out null tracks (Ghost songs)
    return data.items.map((item: any) => item.track).filter((t: any) => t && t.id && t.uri && t.name);
  },
  ...
  getLikedTracksIds: async (limit = 50): Promise<Set<string>> => {
    ...
    return new Set(data.items.map((item: any) => item.track?.id).filter(Boolean));
  },
  ...
  ensureGemsPlaylist: async (): Promise<string> => {
    ...
    const newPlaylist = await SpotifyDataService.createPlaylist(me.id, "GetReady Gems", "A curated collection of your top-tier finds from GetReady syncs.");
  ...
  getPlaylistTracks: async (playlistId: string, limit = 50, offset = 0): Promise<SpotifyTrack[]> => {
    ...
    if (!playlistId || playlistId === "Unlinked") throw new Error("Playlist ID is missing.");
    ...
    try {
      ...
      // FIX: Filter out null/bad tracks (Ghost songs)
      return data.items.map((item: any) => item.track).filter((t: any) => t && t.id && t.uri && t.name);
    } catch (e: any) {
      if (e.status === 404) apiLogger.logError(`Playlist ${playlistId} not found (404).`);
      throw e;
    }
  },
  ...
  searchTracks: async (query: string, limit = 50, offset = 0): Promise<SpotifyTrack[]> => {
    ...
    return (data?.tracks?.items || []).filter((t: any) => t && t.id && t.uri);
  },
  ...
  resolveArtistByExactName: async (name: string): Promise<{ artist: SpotifyArtist | null; debug: any }> => {
    ...
    const items = data?.artists?.items ?? [];
    if (items.length === 0) return { artist: null, debug: { url, count: 0 } };
    ...
    if (exactMatches.length > 0) {
      ...
      return { artist: exactMatches[0], debug: { url, count: items.length } };
    }
    return { artist: items[0], debug: { url, count: items.length } };
  },
  ...
  getArtistTopTracks: async (artistId: string, market = "US"): Promise<SpotifyTrack[]> => {
    ...
    return (data.tracks || []).filter((t: any) => t && t.id && t.uri);
  },
  ...
  getAlbumTracks: async (albumId: string): Promise<SpotifyTrack[]> => {
    const data = await SpotifyApi.request(`/albums/${albumId}/tracks?limit=50`);
    return (data.items || []).filter((t: any) => t && t.id && t.uri);
  },
  ...
  getDeepCuts: async (artistId: string, targetCount = 35): Promise<{ tracks: SpotifyTrack[]; debug: any }> => {
    ...
    const buildTrackPool = async (albums: any[]) => {
      ...
      pool.forEach(t => { if (t && !seenUris.has(t.uri)) { uniquePool.push(t); seenUris.add(t.uri); } });
      return uniquePool;
    };
    ...
    for (const track of shuffled) {
      ...
      if (countForAlbum < 3 && albumId !== lastAlbumId) { selected.push(track); albumCounts.set(albumId, countForAlbum + 1); lastAlbumId = albumId; }
    }
    return { tracks: selected, debug: { albumCount: albumMap.size, poolSize: trackPool.length, selectedSize: selected.length, mode: 'Deep Cuts' } };
  },
  ...
  getRecommendations: async (seedArtists: string[], seedTracks: string[], limit = 20, market = "US", seedGenres: string[] = [], targets: any = {}): Promise<SpotifyTrack[]> => {
    ...
    if (validArtists.length + validTracks.length + validGenres.length === 0) return [];
    const params = new URLSearchParams({ limit: normalizeLimit('/recommendations', limit, 20, 100).toString(), market, ...targets });
    if (validArtists.length > 0) params.append('seed_artists', validArtists.slice(0, 5).join(','));
    if (validTracks.length > 0) params.append('seed_tracks', validTracks.slice(0, 5 - validArtists.length).join(','));
    if (validGenres.length > 0) params.append('seed_genres', validGenres.slice(0, 5 - validArtists.length - validTracks.length).join(','));
    try {
  ...
  getShowEpisodes: async (showId: string, limit = 50, market?: string): Promise<SpotifyEpisode[]> => {
    if (USE_MOCK_DATA) {
      return [{ id: 'mock_ep_1', name: 'The Future of AI Design Systems', description: 'Exploring...', release_date: '2024-02-24', duration_ms: 1800000, images: [{ url: 'https://images.unsplash.com/photo-1590602847861-f357a9332bbc?auto=format&fit=crop&q=80&w=300&h=300' }], uri: 'spotify:episode:mock1' }];
    }
  ...
  getPlaylistById: async (playlistId: string): Promise<any> => SpotifyApi.request(`/playlists/${playlistId}`),

  getAlbumById: async (albumId: string): Promise<any> => SpotifyApi.request(`/albums/${albumId}`),

  resolvePlaylistByName: async (targetName: string): Promise<string | null> => {
    let offset = 0;
    while (true) {
      const data = await SpotifyDataService.getUserPlaylists(50, offset);
