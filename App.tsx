      ...
      <InkBackground>
        ...
        <div className="flex flex-col min-h-[100dvh] h-[100dvh] text-[#A9E8DF] relative overflow-hidden no-select">
          <main 
            ...
            style={{ 
              paddingTop: 'calc(env(safe-area-inset-top, 0px) + 32px)', // Increased top breathing room (+12px room)
          ...
          {authReady && !selectedOption && (authStatus !== 'exchanging' && authStatus !== 'waiting') && (
            <nav className="fixed bottom-0 left-0 right-0 bg-black/70 backdrop-blur-3xl border-t border-white/10 flex justify-around items-start z-50 px-8 pt-1.5" style={{ height: 'calc(env(safe-area-inset-bottom, 0px) + 48px)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
...
const TabButton: React.FC<{ label: string; isActive: boolean; onClick: () => void; icon: React.ReactNode; }> = ({ label, isActive, onClick, icon }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-0 transition-all transform active:scale-90 ${isActive ? 'text-palette-pink scale-105' : 'text-zinc-600 opacity-60'}`}>
    {icon}
    <span className={`text-[6.5px] font-black uppercase tracking-[0.2em] ${isActive ? 'text-palette-pink' : 'text-zinc-600'}`}>{label}</span>
  </button>
);
