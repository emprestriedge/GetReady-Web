...
const RunView: React.FC<RunViewProps> = ({ option, rules, onClose, onComplete, initialResult, onResultUpdate, onPlayTriggered, onPreviewStarted, isQueueMode, onRegenerate }) => {
  ...
  const handlePlayMixDirectly = () => {
    if (!result?.tracks || result.tracks.length === 0) return;
    Haptics.heavy();
    const firstTrack = result.tracks[0];
    // Launches the app immediately (Deep Link)
              ...
              {genStatus === 'DONE' && (
                <button 
                  onClick={() => {
                    Haptics.impactAsync(ImpactFeedbackStyle.Light);
                    if (viewMode === 'PREVIEW') {
                      // TOP Header Button: Launches Spotify directly (Deep Link)
                      handlePlayMixDirectly();
                    } else {
                      handleOpenDevicePicker(); 
                    }
                  }}
                  className={`flex-1 relative overflow-hidden px-4 py-2.5 rounded-[20px] active:scale-95 transition-all shadow-xl flex items-center justify-center gap-3 border ${viewMode === 'PREVIEW' ? 'bg-palette-pink/15 text-palette-pink border-palette-pink/40 shadow-palette-pink/20' : 'bg-palette-emerald/15 text-palette-emerald border-palette-emerald/40 shadow-palette-emerald/20'}`}
                >
          ...
          <div className="bg-zinc-900 border border-white/10 rounded-[44px] p-8 w-full max-sm:p-6 w-full max-w-sm flex flex-col gap-6 shadow-2xl animate-in zoom-in duration-300" onClick={foundEvent => foundEvent.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <span className="text-[9px] opacity-70 font-bold">Deep Link App Launch</span>
              </button>
              <button 
